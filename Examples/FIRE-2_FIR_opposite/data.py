...
def normalize_data(data, labels):
    ...
    # normalize input
    data[:,0]  = (data[:,0] - np.mean(data[:,0]))/np.std(data[:,0])  #flux_100
    data[:,1]  = (data[:,1] - np.mean(data[:,1]))/np.std(data[:,1])  #flux_150
    data[:,2]  = (data[:,2] - np.mean(data[:,2]))/np.std(data[:,2])  #flux_185
    data[:,3]  = (data[:,3] - np.mean(data[:,3]))/np.std(data[:,3])  #flux_230
    data[:,4]  = (data[:,4] - np.mean(data[:,4]))/np.std(data[:,4])  #flux_345
    data[:,5]  = (data[:,5] - np.mean(data[:,5]))/np.std(data[:,5])  #flux_460
    data[:,6]  = (data[:,6] - np.mean(data[:,6]))/np.std(data[:,6])  #flux_670
    data[:,7]  = (data[:,7] - np.mean(data[:,7]))/np.std(data[:,7])  #flux_870
...
def read_data(fin, seed, mode, normalize):
    ...
    data = np.vstack([flux_100, flux_150, flux_185, flux_230, flux_345, flux_460, flux_650, flux_870]).T
    ...
    print ("shape of phys_array", np.shape(phys_array))
    labels = phys_array.reshape((phys_array.shape[0],7))
    #labels = fluxes
