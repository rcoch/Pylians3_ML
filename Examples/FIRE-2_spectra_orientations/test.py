...
#################################### INPUT ##########################################
# data parameters
fin  = 'my_fire2_data_spectrum_subset_orientations.h5'
seed = 5
...
model = architecture.model_1hl(8, h1, 90, dropout_rate)
...
for x, y in test_loader:
    with torch.no_grad():
        ...
        for i in range(0,len(x)):
            ...
            orientation_here = x[i][7].numpy()
            modelled_spectrum = model(x[i]).numpy()

            plt.plot(wavelengths_correct,model(x[i]),'r-',label='NN estimate')
            plt.plot(wavelengths_correct,y[i],'b-',label='SKIRT output, z='+str(np.round(denormed_redshift_here,2))+', angle '+str(int(orientation_here)))
