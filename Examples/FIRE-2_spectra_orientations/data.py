...
def normalize_data(data, labels):
    ...
    data[:,7] = data[:,7] # don't need to normalise the orientation
...
def read_data(fin, seed, mode, normalize):
    ...
    orientations = f['A_all_orientations'][:];
    ...
    data     = np.vstack([redshift, SFR, SFR_10, SFR_100, M_dust, M_gas, M_star, orientations]).T
